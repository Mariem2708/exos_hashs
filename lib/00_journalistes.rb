tab =["@jcunniet","@PaulLampon","@Aziliz31","@ssoumier","@marionsouzeau","@gaellombart",
...
def mini(tableau)
puts tableau.min_by{|nom|nom.length}
end
...
def first_maj(tableau)
  result = 0
  tableau.each do |nom|     #boucle l'array pour tester la condition a chaque handle ( dans cet exemple "nom")
    if nom =~ /^@[A-Z]/ #on utlise un regex pour trouver les conditions de la question (^@ = commence par @ ; [A-Z]= suivit d'une majuscule)
      result +=1
    end
  end
  puts result
...
def alpha ( order)
order.sort_by{|nom|nom.downcase}
end 
...
def classement (tableau)
  tableau.group_by{|nom,value|nom.length}.sort
 end
